...
def _walk_forward_validate(X: pd.DataFrame, y: pd.Series, returns: pd.Series | None = None) -> dict[str, Any]:
    ...
    for fold in range(VALIDATION_FOLDS):
        ...
        row = {"fold": fold + 1, "accuracy": float(accuracy_score(y_test, y_pred)), "balanced_accuracy": float(balanced_accuracy_score(y_test, y_pred)), "brier": float(brier_score_loss(y_test, p_model))}
        if returns is not None:
            y_ret = returns.iloc[test_start:test_end].to_numpy(dtype=float)
            naive_pred = np.zeros(len(y_ret))
            model_pred = np.zeros(len(y_ret))
            row["model_rmse"] = float(np.sqrt(mean_squared_error(y_ret, model_pred)))
            row["naive_rmse"] = float(np.sqrt(mean_squared_error(y_ret, naive_pred)))
            row["beats_naive"] = bool(row["model_rmse"] < row["naive_rmse"])
        rows.append(row)
...
def _regime(df: pd.DataFrame) -> str:
    ...
    vol = returns.tail(20).std(); long_vol = returns.tail(min(120, len(returns))).std(); trend = df["close"].iloc[-1] / df["close"].tail(min(30, len(df))).mean() - 1
    if long_vol and vol > long_vol * 1.5: return "HIGH_VOLATILITY"
    if trend > 0.002: return "TREND_UP"
...
def forecast_timeframe(df: pd.DataFrame, symbol: str, interval: str, horizon_bars: int = 1) -> dict:
    ...
    frame, X, y = _prepare_supervised(df, horizon_bars)
    if len(X) < MIN_ROWS or y.nunique() < 2: return {"status":"DATA UNAVAILABLE","reason":"INSUFFICIENT_TWO_CLASS_DATA","symbol":symbol,"interval":interval}
    validation = _walk_forward_validate(X, y, frame["future_return"])
    beats = validation.get("beats_naive_rate", 0.0)
    candidate = bool(len(validation["folds"]) >= 3 and beats >= 0.75 and (validation["mean_balanced_accuracy"] or 0) > 0.50 and (validation["mean_brier"] or 1) < 0.25)
    model = HistGradientBoostingClassifier(random_state=42); model.fit(X, y)
